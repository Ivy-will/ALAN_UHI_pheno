...
ALAN = VPNALAN_All(N0,:,:);
LST = VPNTaav_All(N0,:,:);
PHENO = VPNEOS_All7(N0,:,:);%(city,year,layer)
Isa = VPNallISA(N0,:);
Len = length(N0);
...
for city = 1:Len
    ...
    for year = 1:7
        start = 365+(year-1)*365+201;
        enddate = 365+(year-1)*365+260;
...
%     scatter(All1(:,4),All1(:,1),20,[0.2 0.70 0.30],'o');
    ...
    ylabel('EOS (DOY)','FontName','Arial','FontSize',14,'LineWidth',1.5,'FontWeight','bold','color',[0.2 0.70 0.30]);
    yyaxis right
...
%     scatter(All1(:,4),All1(:,2),20,[0.8 0.2 0.1],'o');
    ...
    set(gca,'color','none','ycolor',[0.8 0.2 0.1],'FontName','Times New Roman','FontSize',14,'LineWidth',1.5,'FontWeight','bold');
...
%     scatter(All1(:,4),All1(:,3),20,[0.35 0.20 0.58],'o');
    ...
    if R2 < 0.001
        text(min(ISA)+3,max(c_T)+0.2,strcat(' y = ',st2,' p < 0.001 '),'FontName','Arial','FontSize',14,'LineWidth',1.5,'FontWeight','bold','color',[0.8 0.2 0.1]);
    else
        text(min(ISA)+3,max(c_T)+0.2,strcat(' y = ',st2,' R^2 = ',num2str(round(R2,3))),'FontName','Arial','FontSize',14,'LineWidth',1.5,'FontWeight','bold','color',[0.8 0.2 0.1]);
    end
    if t == 0
