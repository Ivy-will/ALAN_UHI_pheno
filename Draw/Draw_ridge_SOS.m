...
[M,N] = find(VPNAllcity(:,8)==3);
TempAllCity = VPNAllcity(M,:);
ALAN0 = VPNALAN_All(M,:,:);
LST0 = VPNLST_All(M,:,:);
PHENO0 = VPNSOS_All7(M,:,:);%(city,year,layer)
Isa0 = VPNallISA(M,:);
...
data2 = zeros(20,10);
data3 = zeros(100,10);
x1 = linspace(40,140,26);
x2 = linspace(-9,24,21);
x3 = linspace(0,100,101);
for i = 1:10
    temp = avPh(:,i);
    data1(:,i) = histcounts(temp,x1);
end
...
xlabel('SOS (DOY)','FontName','Arial','FontSize',36,'LineWidth',1.5,'FontWeight','bold');
...
joyPlot(data2,X2,0.1, ...
       ...
       'FaceAlpha',0.7)
% hTitle = title('Ridgeline Plot');
set(gca,'XTick',(-9:9:24),'FontName','Arial','FontSize',24,'LineWidth',1.5,'FontWeight','bold');
set(gca,'FontName','Arial','FontSize',36,'LineWidth',1.5,'FontWeight','bold');
