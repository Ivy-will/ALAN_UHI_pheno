...
ALAN = ALAN_All;
LST = avTav_EOS;
PHENO = EOS_All;%(city,year,layer)
Isa = AllISA;
Len = length(EOS_All(:,1));

%select T before multiply average Greenup date with a period of 40 days
selectALAN = zeros(Len,7,3,10);
for city = 1:Len    
    for year = 1:7
    selectALAN(city,year,:,:) = ALAN_All(city,(year-1)*12+7:(year-1)*12+9,:);
    end
end
...
%     scatter(All1(:,4),All1(:,1),20,[0.2 0.70 0.30],'o');
    ...
    ylabel('EOS (DOY)','FontName','Arial','FontSize',14,'LineWidth',3,'FontWeight','bold','color',[0.20 0.46 0.22]);
    yyaxis right
